...
from intake.models import Location, Organization, PointOfContact, RequestQueue
...
class OrganizationDetailView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        parent = get_object_or_404(self.parent, id=self.kwargs['org_id'])
        children = self.model.objects.filter(organization=parent)
        return children
    ...
    def get_context_data(self, **kwargs):
        ...
        context['org'] = get_object_or_404(self.parent, id=self.kwargs['org_id'])
        return context
