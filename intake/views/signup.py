...
class ActivateAccount(View):
    ...
    def get(self, request, uidb64, token, *args, **kwargs):
        ...
        if user is not None and account_activation_token.check_token(user, token):
            ...
            # determine if user is site_coordinator and if so
            # determine if SC can create organizations
            if user.profile.role == 'site_coordinator' and Organization.objects.count() == 0:
                user.profile.can_create_organization = True
                user.save()
