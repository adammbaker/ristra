...
from intake.forms.intakebuses import LodgingForm
...
def intake_buses(request):
    ...
    if request.method =='POST':
        ...
        form = LodgingForm()
    return render(request, 'intake/lodging-add.html', {'form': form})
    return HttpResponse("This is what you see on the lodging view and when there are lodgings.")

class LodgingAddPageView(LoginRequiredMixin, TemplateView):
    template_name = "intake/lodging-add.html"

    def get_context_data(self, **kwargs):
        context = super(LodgingAddPageView, self).get_context_data(**kwargs)
        context['form'] = LodgingForm()
